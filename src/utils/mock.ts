...
import { ReferenceType, RoleType, TaskItemType, TaskPhaseStatus, TaskPriority, TaskStage, TaskStatus } from "@/types";
import {
...
const normalizeTask = (task: TaskSeed | Task): Task => {
    const itemType = task.itemType || TaskItemType.Task;
    const existingPhases = normalizeTaskPhases(task.phases);
    const phases = itemType === TaskItemType.Requirement
        ? []
        : existingPhases.length > 0
            ? existingPhases
            : [createLegacyTaskPhase(task.stage || TaskStage.Filed, task.assigneeId || null, task.status || TaskStatus.Todo)];
    const currentPhase = getCurrentTaskPhase(phases);
    const stage = itemType === TaskItemType.Requirement
        ? TaskStage.Filed
        : deriveStageFromPhase(currentPhase, task.stage || TaskStage.Filed);
    const status = itemType === TaskItemType.Requirement
        ? task.status
        : deriveStatusFromPhases(phases, task.status || TaskStatus.Todo);
    return {
        ...task,
        itemType,
        parentRequirementId: itemType === TaskItemType.Requirement ? null : task.parentRequirementId || null,
        assigneeId: itemType === TaskItemType.Requirement ? null : currentPhase?.assigneeId || task.assigneeId || null,
        dueDate: itemType === TaskItemType.Requirement ? null : task.dueDate,
        stage,
        status,
        phases,
...
const mockTasks: TaskSeed[] = [
    ...
    {
        id: "req-1",
        itemType: TaskItemType.Requirement,
        ...
        references: [
            { type: ReferenceType.Document, url: "https://docs.example.com/world-setting", title: "世界观设定文档" },
        ],
        comments: [
    ...
    // 需求单: 战斗系统开发
    {
        ...
        references: [
            { type: ReferenceType.Design, url: "https://figma.com/combat-design", title: "战斗系统设计稿" },
        ],
        comments: [],
    },
    // 子任务: 战斗引擎开发
    ...
    // 子任务: UI界面设计-角色面板
    {
        ...
        references: [
            { type: ReferenceType.UI, url: "https://figma.com/character-ui", title: "角色面板UI设计" },
        ],
        comments: [],
    },
    ...
    // 需求单: 社交系统开发（包含多种状态的任务）
    {
        ...
        references: [
            { type: ReferenceType.Document, url: "https://docs.example.com/social-system", title: "社交系统设计文档" },
        ],
        comments: [
    ...
    // 子任务2: 商城界面开发（进行中）
    {
        ...
        planningId: "plan-3",
        references: [
            { type: ReferenceType.UI, url: "https://figma.com/shop-ui", title: "商城UI设计稿" },
        ],
        comments: [],
    },
    // 子任务3: 支付接口对接（进行中）
    ...
    // 需求单: 角色美术资源
    {
        ...
        references: [
            { type: ReferenceType.Design, url: "https://figma.com/character-art", title: "角色美术设计稿" },
        ],
        comments: [
    ...
    // 需求单: 开发环境搭建
    {
        ...
        updatedAt: "2026-05-15T16:00:00Z",
        stage: TaskStage.Preliminary,
        planningId: "plan-5",

        references: [
            { type: ReferenceType.Document, url: "https://docs.example.com/ue5-setup", title: "引擎配置指南" },
        ],
        comments: [],
    },
    // 子任务: UE5引擎配置
    ...
    // 需求单: 场景渲染系统
    {
        ...
        references: [
            { type: ReferenceType.Design, url: "https://figma.com/pbr-materials", title: "PBR材质规范" },
        ],
        comments: [],
    },
    // 子任务: PBR材质系统
...
export const mockApi = {
    ...
    deleteTask: (id: string): boolean => {
        ...
        if (task.itemType !== TaskItemType.Requirement) {
            if (task.status === TaskStatus.Abandoned) return true;
            tasks[index] = normalizeTask({
                ...task,
                status: TaskStatus.Abandoned,
                updatedAt: new Date().toISOString(),
